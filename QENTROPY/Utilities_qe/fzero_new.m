...
function [x, fval, info, flg, output] = fzero_new(fun, x0, options = struct ())
...
## Get default options if requested.
  ...
  if (! (sign (fa) * sign (fb) <= 0))
    flg = 1;
   printf("Error! fzero:bracket", "fzero: not a valid initial bracketing");
   return
  endif
  ...
  while (niter < maxiter && nfev < maxfev)
    ...
    elseif (fc == 0)
      ...
      flg = 2;
      printf("Error! fzero:bracket", "fzero: zero point is not bracketed");
      return
    endif
...
function fx = guarded_eval (fun, x)
  ...
  if (! isreal (fx))
    flg = 3;
    printf("Error fzero:notreal", "fzero: non-real value encountered");
    return
  elseif (isnan (fx))
    flg = 4;
    printf("Error! fzero:isnan", "fzero: NaN value encountered");
    return
  endif
endfunction
